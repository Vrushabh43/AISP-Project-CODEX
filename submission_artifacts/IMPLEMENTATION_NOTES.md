...
## Required Assets For Full Reproduction

Full reproduction expects the following Hugging Face assets in a local cache:
...
These assets are intentionally not shipped in the compact submission package.
The package keeps source code, prompt/reference files, verification notes,
summary CSVs, and final figures, while excluding large model and adapter
weights. For heavy reproduction, set `HF_HUB_CACHE` and `TRANSFORMERS_CACHE` to
the cache containing those assets.
